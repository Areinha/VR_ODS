...
{
    // CameraPC should only be compiled in the Editor.
    //
    ...
    // queries the camera pose during Update or LateUpdate after CameraPC has been
    // updated will get the wrong value applied by CameraPC intsead.
    ...
    private static CameraPC instance;
    ...
    public static CameraPC Instance
    {
        get
        {
            if (instance == null && !instanceSearchedFor)
            {
                instance = FindObjectOfType<CameraPC>();
 ...
 */
 ...
 //           GvrCursorHelper.HeadEmulationActive = false;
    ...
    private void Update()
    {
        // GvrControllerInput automatically updates CameraPC.
        // This guarantees that CameraPC is updated before anything else responds to
